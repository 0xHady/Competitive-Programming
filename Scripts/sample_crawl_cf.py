#Original Script: https://github.com/tonghuikang/codecomp
...
def parse_url(url, retries=RETRY_COUNT):
    ...
    for problem in soup.find_all(class_="problem-statement"):
        ...
        question_number = title.split(".")[0].strip().upper()
        ...
        inn = ".in"  
        cpp = ".cpp" 
        with open('/home/ahmed/Mytemplate.cpp',"r") as file:
	        MyTemp = file.read()

        for case_num, (input_,output_) in enumerate(zip(inputs, outputs), start=1):
            with open("{}/{}{}{}{}".format(DIRECTORY, question_number, sep, case_num,inn), "w") as f:
        ...
        for case_num, (input_,output_) in enumerate(zip(inputs, outputs), start=1):
            with open("{}/{}{}".format(DIRECTORY, question_number,cpp), "w") as f:
                f.write(MyTemp)
